#!/usr/bin/env python3
"""PDAC scanpy recipe with cosine metric, NO PCA, leiden res=0.4.

Goes back to cosine on full gene-expression vectors (skips PCA). The
neighbor graph is built directly from the (entity × gene) matrix, so
no information is lost in dimensionality reduction.

Settings:
  normalize_total + log1p  (same as canonical recipe)
  no PCA
  sc.pp.neighbors(n_neighbors=30, metric=\"cosine\", use_rep=\"X\")
  sc.tl.umap(min_dist=0.1)
  sc.tl.leiden(resolution=0.4)

Min_tx: 20 for input/cells, 10 for partials.

Outputs in benchmarks/pdac_full_seq/scanpy_cosine_no_pca_res04_spread2/.
"""
...
OUT_DIR = REPO / "benchmarks" / "pdac_full_seq" / "scanpy_cosine_no_pca_res04_spread2"
...
def _run_pipeline(adata, label):
    print(f"  [{label}] normalize_total + log1p + neighbors(cosine, use_rep=X) "
          f"+ UMAP + leiden ...", flush=True)
    t = time.time()
    ...
    sc.tl.umap(adata, min_dist=UMAP_MIN_DIST, spread=2.0, random_state=RNG)
    sc.tl.leiden(
...
def _plot_pair(adata, name, out_path):
    ...
    plt.suptitle(f"PDAC scanpy + cosine neighbors (NO PCA), "
                  f"n_neighbors={N_NEIGHBORS}, UMAP min_dist={UMAP_MIN_DIST}, spread=2.0, "
                  f"leiden res={LEIDEN_RES}",
                  fontsize=12, y=1.0)
    plt.tight_layout()
...
def main() -> int:
    ...
    if adatas:
        ...
        plt.suptitle(f"PDAC scanpy + cosine (NO PCA) — n_neighbors={N_NEIGHBORS}, "
                      f"UMAP min_dist={UMAP_MIN_DIST}, spread=2.0, leiden res={LEIDEN_RES}",
                      fontsize=12, y=1.0)
        plt.tight_layout()
