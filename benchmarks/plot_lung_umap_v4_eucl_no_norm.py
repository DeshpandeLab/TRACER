#!/usr/bin/env python3
"""Lung UMAP v4: euclidean, NO row normalization.

This is the most-likely-to-be-different variant. With no L2 norm and
euclidean distance, the absolute magnitude of each entity's count
vector becomes the dominant signal — big cells are far from small
cells just because of total counts, regardless of composition.

Expectation: the layout will have a clear "size axis" — log10(n_tx)
should look like a continuous gradient across the embedding, not a
broken-up pattern as in cosine/Hellinger variants.

  - log1p of counts (so magnitude is roughly log-scale, otherwise the
    top 1% counts entirely dominate)
  - NO row normalization
  - NO SVD (all 300 dims)
  - metric=euclidean
  - n_neighbors=15, min_dist=0.1

Outputs in benchmarks/lung_full_seq/v4_eucl_no_norm/.
"""
...
OUT_DIR = REPO / "benchmarks" / "lung_full_seq" / "v4_eucl_no_norm"
...
def _preprocess(m):
    """log1p only — no row normalization, no SVD, raw magnitudes preserved."""
    m = m.copy()
    m.data = np.log1p(m.data)
    return m


def _umap_eucl(mat, label):
    print(f"  [{label}] UMAP (d={mat.shape[1]}, "
          f"n_neighbors={UMAP_NEIGHBORS}, min_dist={UMAP_MIN_DIST}, "
          f"metric=euclidean, no norm, no SVD) ...", flush=True)
    t = time.time()
...
def main() -> int:
    ...
    for name, mask, ent_col in [
        ...
        if M.shape[0] < UMAP_NEIGHBORS + 1:
            ...
            continue
        Mp = _preprocess(M)
        emb = _umap_eucl(Mp, name)
        _plot(emb, np.log10(sizes), "log10(n_tx)",
               f"Lung {name} — n={len(ents):,}, d={M.shape[1]}  "
               f"(min_tx={MIN_TX}, n_neighbors={UMAP_NEIGHBORS}, "
               f"min_dist={UMAP_MIN_DIST}, euclidean NO norm, NO SVD)",
               OUT_DIR / f"umap_v4_{name}.png")
    ...
    if len(out_records) >= 2:
        ...
        for ax, rec in zip(axes, out_records):
            ...
            ax.set_xticks([]); ax.set_yticks([])
            ax.spines[["top", "right"]].set_visible(False)
        plt.suptitle(f"Lung UMAP v4 — euclidean NO row-norm, NO SVD, "
                      f"n_neighbors={UMAP_NEIGHBORS}, min_dist={UMAP_MIN_DIST}",
                      fontsize=12, y=1.0)
        plt.tight_layout()
        combined = OUT_DIR / "umap_v4_combined.png"
        plt.savefig(combined, dpi=110, bbox_inches="tight")
        plt.close(fig)
        print(f"\n  -> {combined}", flush=True)

    pd.concat(out_records, ignore_index=True).to_parquet(
        OUT_DIR / "umap_v4_embeddings.parquet", index=False)
