#!/usr/bin/env python3
"""Lung UMAP v7: correlation metric, no SVD, all genes.

The correlation metric mean-centers each row before computing cosine,
so it factors out a per-entity "average expression magnitude" baseline.
This removes the top-PC direction that's been dominating prior variants
(everyone-expresses-housekeeping-genes axis) and forces UMAP to layout
by *relative* expression pattern.

  - log1p of counts
  - NO row normalization (correlation centers internally)
  - NO SVD (all 300 dims)
  - metric="correlation"
  - n_neighbors=15, min_dist=0.1

Outputs in benchmarks/lung_full_seq/v7_correlation/.
"""
...
OUT_DIR = REPO / "benchmarks" / "lung_full_seq" / "v7_correlation"
...
def main() -> int:
    ...
    if len(out_records) >= 2:
        ...
        for ax, rec in zip(axes, out_records):
            ...
            ax.set_xticks([]); ax.set_yticks([])
            ax.spines[["top", "right"]].set_visible(False)
        plt.suptitle(f"Lung UMAP v7 — correlation metric, NO SVD, all genes, "
                      f"n_neighbors={UMAP_NEIGHBORS}, min_dist={UMAP_MIN_DIST}",
                      fontsize=12, y=1.0)
        plt.tight_layout()
        combined = OUT_DIR / "umap_v7_combined.png"
        plt.savefig(combined, dpi=110, bbox_inches="tight")
        plt.close(fig)
        print(f"\n  -> {combined}", flush=True)

    pd.concat(out_records, ignore_index=True).to_parquet(
        OUT_DIR / "umap_v7_embeddings.parquet", index=False)
