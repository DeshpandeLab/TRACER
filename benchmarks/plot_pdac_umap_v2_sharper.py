#!/usr/bin/env python3
"""UMAP v2: sharper params + no row normalization.

Recomputes UMAP for THREE entity subsets independently so each gets a
full 2D canvas:
  1. INPUT cell_id (Xenium segmentation)
  2. SEG output — cells only   (_etype == "cell")
  3. SEG output — partials only (_etype == "partial")

Changes vs v1:
  - Row normalization: NONE (was L2)
  - n_neighbors:        15      (was 30)
  - min_dist:           0.1     (was 0.3)

Outputs land in benchmarks/pdac_full_seq/v2/:
  umap_v2_input.png
  umap_v2_seg_cells.png
  umap_v2_seg_partials.png
  umap_v2_combined.png      (3-panel side-by-side, log10(n_tx))
  umap_v2_embeddings.parquet
"""
...
OUT_DIR = REPO / "benchmarks" / "pdac_full_seq" / "v2"
...
def _preprocess(m: sparse.csr_matrix) -> sparse.csr_matrix:
    """log1p of counts. No row normalization (v2 setting)."""
    m = m.copy()
...
def _pca_umap(mat: sparse.csr_matrix, label: str) -> np.ndarray:
    ...
    print(f"  [{label}] UMAP fit (n_neighbors={UMAP_NEIGHBORS}, "
          f"min_dist={UMAP_MIN_DIST}, metric=cosine) ...", flush=True)
    t = time.time()
    reducer = umap.UMAP(
...
def main() -> int:
    ...
    # Combined 3-panel
    fig, axes = plt.subplots(1, 3, figsize=(22, 7.5), dpi=110)
    for ax, rec in zip(axes, out_records):
        ...
        ax.set_xticks([]); ax.set_yticks([])
        ax.spines[["top", "right"]].set_visible(False)
    plt.suptitle(f"PDAC UMAP v2 — log1p only (no row-norm), "
