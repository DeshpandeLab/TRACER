#!/usr/bin/env python3
"""Lung UMAP via the canonical scanpy recipe from metrics_umap.ipynb.
...
Outputs in benchmarks/lung_full_seq/scanpy_recipe/.
"""
...
OUT_DIR = REPO / "benchmarks" / "lung_full_seq" / "scanpy_recipe"
...
def _run_pipeline(adata, label):
    ...
    sc.tl.umap(adata, random_state=RNG)
    sc.tl.leiden(
...
def _plot_pair(adata, name, out_path):
    ...
    plt.suptitle(f"Lung scanpy recipe — normalize_total + log1p + PCA(300) + "
...
def main() -> int:
    ...
    if adatas:
        ...
        plt.suptitle(f"Lung scanpy recipe (normalize_total + log1p + PCA(300) + "
                      f"n_neighbors={N_NEIGHBORS} + UMAP default)",
                      fontsize=12, y=1.0)
        plt.tight_layout()
