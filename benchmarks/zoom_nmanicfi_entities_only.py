...
associated with cell `nmanicfi-1`.

    SEG side:    `nmanicfi-1` (main) + `nmanicfi-1-1` (Phase 1c partial)
    NOSEG side:  every cascade entity that has ≥1 tx in nmanicfi-1's
...
def main() -> int:
    ...
    # SEG entities for this cell: nmanicfi-1 + nmanicfi-1-1
    SEG_LABS = {"nmanicfi-1", "nmanicfi-1-1"}
    # NOSEG entities for this cell: every cascade label that has ≥1 tx
    # in nmanicfi-1's Xenium footprint
    in_cell = df[df["cell_id"] == "nmanicfi-1"]
    NOSEG_LABS = set(in_cell["noseg_lab"].astype(str).unique()) - SENT
    print(f"SEG entities for nmanicfi-1: {len(SEG_LABS)} → {sorted(SEG_LABS)}")
    print(f"NOSEG entities touching nmanicfi-1: {len(NOSEG_LABS)} → "
    ...
    _draw_panel(axes[1, 1], noseg_df, "noseg_lab",
                  ...
                  (xmin, xmax), (ymin, ymax), G=1.0)
    plt.suptitle(
        "Entities associated with cell nmanicfi-1 — final pipeline outputs\n"
        "left: SEG    right: NOSEG    top row: 2 µm tiles    bottom row: 1 µm tiles",
        fontsize=14, y=1.0,
    )
    plt.tight_layout()
    out = ZOOM_DIR / "zoom_nmanicfi_entities_only.png"
