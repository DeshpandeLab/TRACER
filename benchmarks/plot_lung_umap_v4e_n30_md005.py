#!/usr/bin/env python3
"""Lung UMAP v4b: same as v4 (euclidean, no norm, no SVD) but n_neighbors=30, min_dist=0.3.

min_dist=0.1 gives tight clusters; min_dist=0.3 spreads them out
into a smoother layout where macro topology is easier to see at a
glance. Useful for getting overall manifold shape vs sub-cluster
detail.
"""
...
OUT_DIR = REPO / "benchmarks" / "lung_full_seq" / "v4e_n30_md005"
OUT_DIR.mkdir(parents=True, exist_ok=True)

MIN_TX = 20
UMAP_NEIGHBORS = 30
UMAP_MIN_DIST = 0.05   # ← only change vs v4
RNG = 42
...
def main() -> int:
    ...
    if len(out_records) >= 2:
        ...
        for ax, rec in zip(axes, out_records):
            ...
            ax.set_xticks([]); ax.set_yticks([])
            ax.spines[["top", "right"]].set_visible(False)
        plt.suptitle(f"Lung UMAP v4b — euclidean NO row-norm, NO SVD, "
                      f"n_neighbors={UMAP_NEIGHBORS}, min_dist={UMAP_MIN_DIST}",
                      fontsize=12, y=1.0)
        plt.tight_layout()
        combined = OUT_DIR / "umap_v4e_combined.png"
        plt.savefig(combined, dpi=110, bbox_inches="tight")
        plt.close(fig)
        print(f"\n  -> {combined}", flush=True)

    pd.concat(out_records, ignore_index=True).to_parquet(
        OUT_DIR / "umap_v4e_embeddings.parquet", index=False)
