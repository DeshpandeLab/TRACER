...
OUT_DIR = REPO / "benchmarks" / "pdac_full_seq" / "scanpy_recipe_res015_partial10"
...
def _run_pipeline(adata, label):
    ...
    sc.tl.umap(adata, min_dist=0.1, random_state=RNG)
    sc.tl.leiden(
...
def main() -> int:
    ...
    if adatas:
        ...
        plt.suptitle(f"PDAC scanpy recipe (normalize_total + log1p + PCA(300) + "
                      f"n_neighbors={N_NEIGHBORS} + UMAP min_dist=0.1)",
                      fontsize=12, y=1.0)
        plt.tight_layout()
